...
async function main() {
    try {
        ...
        contract.onUpdate(({ status, payout, bid_price }) => {
            switch (status) {
                case "proposal":
                    return console.log(`Current payout: ${payout.currency} ${payout.display}`);
                case "open":
                    return console.log(`Current bid price: ${bid_price.currency} ${bid_price.display}`);
                default:
                    break;
            }
        });
        ...
        await contract
            .onUpdate()
            .pipe(find(({ payout }) => payout.value >= expected_payout))
            .toPromise();
